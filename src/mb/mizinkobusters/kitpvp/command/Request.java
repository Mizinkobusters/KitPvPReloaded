				...
				if(Bukkit.getPlayerExact(args[0]).getName() != null
					...
					for(Team teams : sb.getTeams()) {
						if(teams.getEntries().size() == 0
								&& !teams.hasEntry(player.getName())
								&& !teams.hasEntry(Bukkit.getPlayerExact(args[0]).getName())) {
							player.sendMessage(prefix + "§e§o" + Bukkit.getPlayerExact(args[0]).getName() + " §aにチーム申請を送信しました");
							Bukkit.getPlayerExact(args[0]).getPlayer().sendMessage(prefix + "§6§o" + player.getName() + " §aからチーム申請が送信されました");
							Bukkit.getPlayerExact(args[0]).getPlayer().sendMessage(prefix + "§a申請を受諾するには §e/accept §aを実行してください");
							Bukkit.getPlayerExact(args[0]).getPlayer().sendMessage(prefix + "§a申請は20秒後に自動的に破棄されます");

							send.put(Bukkit.getPlayerExact(args[0]).getUniqueId(), player.getUniqueId());
